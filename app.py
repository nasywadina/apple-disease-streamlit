...
# ------------------------------------------------------------------------------
# 2. CUSTOM CSS (STYLING MODERN & RESPONSIF)
# ------------------------------------------------------------------------------
st.markdown(
    """
    <style>
...
# Pilihan metode input gambar dalam satu halaman
...
st.sidebar.markdown(
    ...
    <div style='font-size: 12px; color: #64748B;'>
        ...
        <b>Mode:</b> Single Page Application (SPA)
    </div>
    """,
    unsafe_allow_html=True,
)
...
# ------------------------------------------------------------------------------
# 5. SINGLE PAGE APPLICATION (HOME + DETEKSI DALAM 1 HALAMAN)
# ------------------------------------------------------------------------------

# --- HEADER & INFORMASI SINGKAT ---
st.title("🍎 Sistem Klasifikasi Penyakit Buah Apel")
st.write(
    "Deteksi kondisi kesehatan buah apel secara presisi dan instan menggunakan Deep Learning (MobileNetV2)."
)

# Expander Ringkasan Informasi / Petunjuk
with st.expander("📖 **Panduan Sistem & Penjelasan Aplikasi (Klik untuk Buka/Tutup)**", expanded=False):
    ...
    with col_b:
        ...
        st.write(
            ...
            2. Unggah gambar atau tangkap foto buah apel melalui kamera.
            3. Sistem secara otomatis akan menampilkan diagnosa dan persentase probabilitas.
            """
        )

st.markdown("---")

# --- CORE SPA INTERFACE (INPUT & OUTPUT SIDE-BY-SIDE) ---
...
with col_input:
    ...
    if input_mode == "📁 Upload File (JPG/PNG)":
        ...
        if uploaded_file is not None:
            ...
            st.image(image_source, caption="Gambar yang Diunggah", use_container_width=True)
        else:
            st.info("Silakan unggah gambar buah apel untuk memulai analisa.")

    elif input_mode == "📸 Real-time Kamera (Webcam)":
        camera_file = st.camera_input("Arahkan kamera ke buah apel & ambil foto:", key="camera_input")
        if camera_file is not None:
            image_source = Image.open(camera_file)
            st.image(image_source, caption="Hasil Tangkapan Kamera Real-time", use_container_width=True)
        else:
            st.info("Izinkan akses kamera pada browser Anda lalu ambil foto objek.")

# -------------------------- OUTPUT PANEL (KANAN) --------------------------
with col_output:
    st.markdown('<div class="pro-card-header"><h3>📊 Hasil Diagnosa</h3></div>', unsafe_allow_html=True)
    ...
    if image_source is not None:
        if model is None:
            ...
            if confidence < CONFIDENCE_THRESHOLD:
                ...
                if detail:
                    with st.expander("📌 **Deskripsi Penyakit & Solusi Penanganan**", expanded=True):
                        st.markdown(f"**{detail.get('title', '')}**")
                        st.write(detail.get("desc", ""))
                        st.markdown("**Langkah Penanganan:**")
                        st.write(detail.get("prevention", ""))
    else:
        st.write("Hasil diagnosa dan bar probabilitas akan langsung muncul di panel ini setelah Anda memberikan input gambar.")
